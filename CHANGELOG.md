...
- **"FX Transfer In" / "FX Transfer Out" trade types** — An internal conversion between an account's own AUD and USD cash sleeves (Moomoo's "Currency Exchange" line) was previously recorded as a plain `deposit`/`withdrawal`, identical to real bank transfers. Since it's not new external capital, this silently inflated/deflated `net_deposited` in every return calculation that uses it (TWR external flows, Modified Dietz monthly flows, drawdown, group statistics) — a portfolio's own currency-conversion housekeeping was making its reported gain % look better or worse than reality. Cash balance itself was always correct; only the "how much did I actually put in" tracking was wrong. New `fx_transfer_in`/`fx_transfer_out` trade types move cash exactly like before but are excluded from deposited/withdrawn and external-flow calculations everywhere they're used (`backend/src/services/calculations/holdings.ts`, `backend/src/routes/reports.ts`, `backend/src/routes/groups.ts`), while the Cash Flows report (`frontend/src/pages/reports/CashFlowPage.tsx`) and both `/reports/cash-flows` endpoints still show them under their existing "FX Transfer" cards. The Moomoo monthly-PDF and annual-summary parsers (`moomoo.ts`, `moomoo-xlsx.ts`) both classify Currency Exchange / internal-transfer lines this way now. **Migration `012_fx_transfer.sql` must be run before deploying this change** — it extends the `trade_type` CHECK constraint and reclassifies existing rows (44 found across both portfolio groups, dating back to Dec 2024) that were recorded as deposit/withdrawal but are actually FX transfers. Found while reviewing the Personal Investment August 2026 import, where a real bank withdrawal was about to be conflated with a same-day currency conversion.

### Fixed

- **Moomoo PDF import — "Bank Transfer Withdrawals" silently dropped** — `parseCashSection`'s (`backend/src/services/pdf-parser/moomoo.ts`) line-matching regex only recognized the cash-flow Type string `"Bank Transfer Deposits"`; Moomoo labels an outbound transfer `"Bank Transfer Withdrawals"` instead, so the whole line failed to match and was silently excluded — no warning, no trace. Found while reviewing the Personal Investment August 2026 statement, which had a $5,727.00 AUD withdrawal that never reached the import preview. The regex and branch handling now recognize both Deposits and Withdrawals.
- **Moomoo PDF import — every dividend/interest/other-income row created a duplicate security keyed by currency instead of exchange** — Securities are uniquely keyed by `(symbol, exchange)` on upsert, but `parseCashSection` (`backend/src/services/pdf-parser/moomoo.ts`) set a cash-line trade's `exchange` field to the transaction's currency code (e.g. `"USD"`) rather than a real exchange (`"US"`). Every dividend therefore resolved to a phantom duplicate security distinct from the one its buy/sell trades use — e.g. a real "MU" holding on exchange `US` plus an invisible, disconnected "MU" dividend history on exchange `USD`. Now maps currency to the exchange it actually trades on in this account (AUD→ASX, USD→US, HKD→HK) for any row referencing a real security, leaving the `CASH` placeholder symbol (deposits/withdrawals/FX transfers — not a real holding) unaffected. Found while reviewing the July 2026 USD import preview; historical duplicates already created for ATOM, BITU, and FANG were corrected directly in the database (4 dividend trades repointed to their correct security, 3 phantom security rows deleted).
- **Moomoo PDF import — DRIP share movements created a duplicate security and were misclassified as an outright purchase** — `parseMovementSection` (`backend/src/services/pdf-parser/moomoo.ts`) only recognized a movement's exchange/security-name context line when it contained the literal word "Other" (the Type shown for Gift Share promotions). A Dividend Reinvestment Plan (DRIP) movement's context line instead reads "Asset Adjustment", so the exchange was never captured and silently fell back to a hardcoded default of `US` — creating a second, phantom security record for a stock already held on `ASX` instead of adding to the existing holding. Detection is now based on the presence of an exchange code (`US`/`ASX`/`HK`) rather than a specific Type string. Separately, a comment containing "DRIP" is now classified as `trade_type: 'drp'` instead of `'buy'`, so it can be distinguished from an outright purchase in reports — the statement carries no dollar figure for the reinvested amount, so the true dividend income and cost base still need manual entry (the per-unit distribution rate can be checked against the fund's own dividend history). Verified against a real statement that had created a duplicate GDX security; the DB rows affected by this import were corrected directly (repointed to the real security, split into a dividend + drp pair).
...
- **Reconcile Against Moomoo — flagged manually-consolidated multi-fill orders as discrepancies** — A single order that fills across several partial executions can be recorded as one aggregated row on one side (e.g. a manually-entered trade merging same-price fills into one line — confirmed against real data by notes like "Manual" or "Update from 100 to 200 manually" on such rows) and as separate line items on Moomoo's file. The reconcile tool's 1:1 matching could never match these individually, so it flagged financially-identical trades as missing/unexpected. Added a second matching pass: leftover entries for the same (type, symbol) are date-clustered, and a cluster reconciles as a whole if the totals on both sides tie out exactly — shown in the UI as "matched as a consolidated order" rather than a discrepancy. Verified against a real annual export that surfaced exactly this case (two ASTS orders, 70 and 200 shares) — now correctly reconciles with zero discrepancies. (`backend/src/services/reconcile/annualReconcile.ts`, `frontend/src/pages/groups/GroupReconcilePage.tsx`)
- **Group Tax Report — dividends/interest converted at today's rate instead of each payment's own date** — The Tax Summary previously converted all foreign-currency dividends and interest to the base currency using a single flat "today" exchange rate, while the exported Trade Ledger converted each payment at its own transaction-date rate — so the same income showed two different totals depending on which part of the same report you read. Now both use the payment's own date, the same treatment already used for capital gains' disposal-date rate, so the Tax Summary and Trade Ledger always agree. A tooltip on the Dividends/Interest/Other Income stat cards (`GroupTaxPage.tsx`) explains the methodology. (`backend/src/services/reports/groupTax.ts`)
- **Moomoo annual "Financial Year Summary" parser — dividends double-counted and understated** — `parseMoomooAnnualSummary` (`backend/src/services/pdf-parser/moomoo-xlsx.ts`) used dividends' net-of-withholding-tax amount instead of the gross amount (understating assessable foreign dividend income, since AU tax residents gross up and claim a separate foreign-income-tax-offset rather than reducing income by the tax withheld), and would additionally re-emit every dividend a second time from the Cash Overview sheet's "CASH DIVIDEND"/"WITHHOLDING TAX" lines as generic deposits/withdrawals — silently double-counting income if this file were ever imported. Verified against a real full-FY export: reconciled all 215 buy/sell trades, 3 dividends, and 46 deposits/withdrawals to the cent against the database (two cosmetic-only quirks aside — Moomoo's own file rounds displayed "Avg Price" more coarsely than the actual transaction amount, and reports a handful of early-AEST-morning trades one calendar day earlier, apparently using UTC internally).
...
- **Reconcile Against Moomoo** — New group-level report (`/groups/:id/reconcile`, linked from the Group Dashboard) that uploads Moomoo's annual "Financial Year Summary" .xlsx export and compares it against what's actually saved in the database — a read-only check, nothing is imported or modified. The single file covers the whole account (both AUD and USD legs); each portfolio in the group is automatically matched against the slice of the file in its own currency, the same way monthly PDF imports are currency-filtered per portfolio. Reports a clear pass/fail per portfolio plus itemized tables for anything in the file but missing from the database, anything in the database but missing from the file, and (informational only) trades that matched but landed on a different calendar date — this last case is expected for early-AEST-morning trades, where Moomoo's annual export appears to timestamp in UTC while the monthly statements it's derived from use local time. Formalizes the manual reconciliation process used to validate the FY2025-26 fixes above; verified against a real annual export with zero discrepancies on both portfolios. (`backend/src/services/reconcile/annualReconcile.ts`, `backend/src/routes/groups.ts`, `frontend/src/pages/groups/GroupReconcilePage.tsx`, `frontend/src/hooks/useGroupReports.ts`)
- **"Other Income" trade type** — Broker referral/incentive cash rewards (Moomoo's "Stock Cash Coupon" and "Cash Voucher") were previously imported as `interest`, which overstated interest income and disagreed with Moomoo's own annual tax summary (which reports these under neither Dividends nor Interest — its Interest total was correctly $0.00). New `other_income` trade type, added to the tax calculations (individual and group), the Group Tax Report export, the Trades page filter, and the manual trade-entry form. **Migration `011_other_income.sql` must be run before deploying this change** — it extends the `trade_type` CHECK constraint and reclassifies existing coupon/voucher rows that were previously stored as `interest`. (`supabase-central/migrations/011_other_income.sql`, `backend/src/services/pdf-parser/moomoo.ts`, `backend/src/services/reports/groupTax.ts`, `backend/src/routes/reports.ts`, `backend/src/routes/groups.ts`, `frontend/src/pages/reports/TaxPage.tsx`, `frontend/src/pages/groups/GroupTaxPage.tsx`, `frontend/src/pages/TradesPage.tsx`, `frontend/src/components/forms/TradeForm.tsx`)
- **Group Tax Report export (XLSX / PDF)** — The consolidated Group Tax Report (`/groups/:id/tax`) now has **XLSX** and **PDF** export buttons next to the FY filters. New endpoint `GET /api/groups/:id/tax/export?fyStart=&year=&format=xlsx|pdf` returns:
  - **Tax Summary** — same totals as the on-screen report (dividends, interest, short/long-term CGT, CGT discount, taxable income), plus the per-portfolio breakdown.
  - **Trade Ledger** — every trade across all portfolios in the group for the selected FY, each converted to the group's base currency at **that trade's own trade-date FX rate** (not today's rate), so historical conversions stay accurate regardless of when the report is generated.
  - **Capital Gains** — the flattened FIFO CGT lot detail (buy/sell dates, cost base, proceeds, discount eligibility, net gain) converted at each lot's disposal-date rate, matching `/api/groups/:id/capital-gains`.
  - The existing `/api/groups/:id/tax` summary route was refactored to share this same computation (`backend/src/services/reports/groupTax.ts`) rather than duplicating the FX/CGT logic — its response shape is unchanged. XLSX generation reuses the existing `xlsx` dependency; PDF generation adds `pdfkit`. (`backend/src/services/reports/groupTax.ts`, `backend/src/services/export/groupTaxExport.ts`, `backend/src/routes/groups.ts`, `frontend/src/pages/groups/GroupTaxPage.tsx`, `frontend/src/hooks/useGroupReports.ts`)
...
- **Target Portfolio holdings — dollar allocation column** — The detail/edit page header now has an **Investable** field (default `$100,000`, persisted per-portfolio in `localStorage` under `folio_target_investable_<id>`) representing hypothetical capital to deploy. A new read-only **Alloc $** column shows each row's `allocation_pct / 100 × investable`, and the holdings header shows the running allocated-dollar total against the investable figure. Pure `allocationValue` helper with unit coverage. (`frontend/src/pages/targets/TargetPortfolioDetailPage.tsx`, `frontend/src/pages/targets/sortDraftItems.test.ts`)
...
- **Cash Flow report** — New page under Reports showing all deposits and withdrawals for any date range (YTD / 1Y / 2Y / All / Custom). Transactions are auto-classified from their notes: "Bank Transfer" and "FX Transfer" are detected by keyword; anything else falls into "Other". "Bank Transfer" and "Other" deposits are combined into a single **Bank Transfer Deposits** card (both represent cash-in from your bank — e.g. Zepto payments are classified the same as labelled bank transfers). FX transfers are grouped by **direction** (e.g. "FX Transfer (USD → AUD)", "FX Transfer (AUD → USD)") — direction is parsed from the notes field so new currency pairs appear automatically. Each direction card shows the deposit side (what arrived in the destination currency). All three cards include a hover tooltip explaining what is included. A category filter dropdown lets users drill into each type. A "Category" column is shown in the table. Supports Individual / Group toggle; Group view converts all amounts to the group base currency and shows a Portfolio column. (`backend/src/routes/reports.ts`, `backend/src/routes/groups.ts`, `frontend/src/pages/reports/CashFlowPage.tsx`, `frontend/src/hooks/useReports.ts`, `frontend/src/hooks/useGroupReports.ts`, `frontend/src/components/layout/Sidebar.tsx`)
...
- **Portfolio switch via selector leaves stale data on all portfolio-specific pages** — Switching portfolios via the top-bar selector only updated `activePortfolio` in React context; it never navigated. The URL stayed at the old portfolio's route (e.g. `/portfolios/AUD_ID/holdings`), so `useParams` kept returning the old ID. Pages derive their data-fetch key from `id || activePortfolio?.id` — since the URL param is always truthy, `activePortfolio` was silently ignored for data. The result: the subtitle updated to the new portfolio name (from context) but all data remained from the previous portfolio. Fixed by navigating to the equivalent sub-path for the new portfolio whenever the switch happens on a portfolio-specific route (e.g. switching while on Holdings navigates to the new portfolio's Holdings). Dashboard, Groups, and Settings pages are unaffected. (`frontend/src/components/layout/PortfolioSelector.tsx`)
...
- **Group performance chart: early oscillations (e.g. −75% in first months) from dropped external flows** — The group TWR was using `extFlowByDate[date]` directly in the per-portfolio `dateMap`. Deposits/withdrawals on non-priceMap dates (weekends, holidays) had no priceMap entry and were silently dropped: `getCashAt()` already reflected the deposit in `totalValue`, but the extFlow neutraliser on the TWR denominator never fired. The next priceMap date would see the cash appear "for free" (no extFlow), inflating the multiplier and producing a large artificial spike. On display re-normalisation the chart start used this inflated multiplier as its baseline — so all subsequent normal market drops appeared as deep losses (up to −75% in the early chart). Fixed by remapping each portfolio's external flows to the next available priceMap date before building the `dateMap`, matching the pattern in `reports.ts` individual TWR. (`backend/src/routes/groups.ts`)

- **Performance chart: stale price cache causes artificial portfolio drop at end of chart** — The `price_history` cache check only validated that the cache started early enough (within 7 days of `fromDate`), but never checked whether the cache was fresh at the trailing end. When a security's cache ended on an older date (e.g. Friday May 29) while another holding had prices through a more recent date (e.g. Monday June 1), the combined `priceMap` included June 1 but the stale holding had no entry for that date. `calculateHoldings` returns `market_value: null` for any holding with no price, which collapses to `0` in the portfolio-value sum — producing an artificial drop in the TWR chart on every date where any holding is cache-stale. Fixed by adding an end-date staleness check: if the cache's last row is more than 3 calendar days before the requested `toDate`, the cache is bypassed and a fresh Yahoo Finance fetch is issued (upsert keeps the existing rows and adds the missing ones). (`backend/src/services/market-data/yahoo.ts`)
