...
- **Group chart phantom +80% jump after null-gap period** — When the TWR chain hit a null gap (e.g. combined value briefly negative during AUD→USD FX transfer), `prevValue` was frozen at the last pre-gap value. On the next day, when the USD deposit arrived as a large positive extFlow, `adjustedBase = frozenPrevValue + USDdeposit` was enormous while `totalValue` reflected the actual recovered combined value — creating a phantom loss. Then when the combined value properly recovered, the factor was huge → the apparent +80% jump. Root fix: **always update `prevValue = totalValue` in the null branch** (even when totalValue is negative). Subsequent extFlows then adjust against the correct baseline, so the chain resumes accurately with no artificial distortion. (`backend/src/routes/reports.ts`, `backend/src/routes/groups.ts`)
