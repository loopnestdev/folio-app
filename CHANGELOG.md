...
- **Group chart: FX-transfer timing mismatch creating large null gap and drop** — A cross-currency FX transfer (AUD→USD) appears as a large negative extFlow on day D1 (AUD withdrawal) and a large positive extFlow on day D2 (USD deposit, typically a different date). With the frozen-prevValue approach, D1 creates a null gap (adjustedBase ≤ 0), and D2 resumes the chain with `adjustedBase = frozen_prevValue + large_positive` far exceeding `totalValue`, producing a large spurious drop (e.g. −57%). Fix: before the TWR loop, scan the combined extFlow series for negative–positive pairs within a 7-day window where the positive is 50%–150% of the absolute negative (the FX rate tolerance range). When found, the negative flow is moved to the deposit date and both are netted. Result: D1 extFlow = 0 (only a 1-day null from totalValue < 0 while USD cash hasn't arrived); D2 extFlow = net FX cost (small negative); TWR factor on D2 ≈ 1.0 — correctly reflecting zero investment return during the internal capital movement. (`backend/src/routes/groups.ts`)
