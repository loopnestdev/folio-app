...
- **Group chart phantom jump after null-gap period** — When the TWR chain hit a null gap, `prevValue` was frozen at the last pre-gap value. If a large positive extFlow (e.g. the USD deposit matching an AUD→USD FX withdrawal) then arrived, `adjustedBase = frozenPrevValue + deposit` was far larger than the actual combined value, producing a phantom loss, which then over-corrected into a phantom gain on recovery. Root fix: split the null-gap branch into two cases: **(A) adjustedBase ≤ 0** (extFlow exceeded portfolio value, e.g. FX withdrawal too large): apply `prevValue += extFlow` so the next incoming extFlow adjusts against the correct post-withdrawal baseline — the matching deposit then sees `adjustedBase ≈ actualValue → factor ≈ 1.0`. **(B) adjustedBase > 0 but totalValue ≤ 0** (portfolio crashed or cash overdraft): freeze prevValue so the chain resumes cleanly when the portfolio recovers. (`backend/src/routes/reports.ts`, `backend/src/routes/groups.ts`)
