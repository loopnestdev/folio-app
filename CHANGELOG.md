...
- **Performance chart: crash to −100% after any buy or sell trade** — Two failure modes with the previous approach, both caused by a price-basis mismatch between `extFlow` (executed prices) and `investedValue` (close prices): (1) buying a position at close price made `numerator = investedValue − extFlow ≈ 0`, driving `factor ≈ 0` and permanently zeroing the multiplier; (2) selling at an executed price below the previous day's close made `adjustedBase` negative. Fixed by storing position changes as `{ qty, symbol }` tuples and pricing them at the **close price on the remapped trading day** when building `extFlow`. With both `extFlow` and `investedValue` on the same close-price basis, the numerator reduces to `(unchanged positions at close)` — always ≥ 0 — and the factor measures only the overnight price return on held positions, which is the correct TWR definition. (`backend/src/routes/reports.ts`)
