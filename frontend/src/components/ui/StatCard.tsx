...
export function StatCard({
  ...
  const trendColor = trend !== undefined ? getValueColor(trend) : 'var(--color-muted)';
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <p className="text-[13px] font-semibold text-[#7a7a7a] uppercase tracking-wide">{label}</p>
        {icon && <div className="text-[#7a7a7a]">{icon}</div>}
      </div>
      ...
      {loading ? (
        ...
        <>
          <p className="text-[28px] font-semibold tracking-tight text-[#1d1d1f] leading-tight">{value}</p>
